...
for e_i in range(epoch):
    cost_factor = CostFactor_1DGS(gaussian_num=3000, is_great_init=False, parameter_space=0)
    ...
    ## LM
    start_time = time.time()
    _, _, LM_solveTime= optimizer_nls.solve(cost_factor, show_process=False, show_result=True)
    end_time = time.time()
    ...
    ## sparse LM
    start_time = time.time()
    _, _, SparseLM_solveTime = optimizer_nls_sparse.solve(cost_factor, block_size=50, show_process=False, show_result=True)
    end_time = time.time()
