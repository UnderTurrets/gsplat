...
class LevenbergMarquardt(Optimizer):
    def __init__(self,
        ...
        print('\033[93m'
              + '''
              The order of parameters correspond to the order of jacobian\'s col!
    ...
    def update(self, group: Dict[str, Any], delta_x: Tensor) -> None:
    ...
    def recover(self, group: Dict[str, Any]):
        ...
        for p in params:
            params_dim += p.numel()
        assert params_dim == params_vector.shape[0], "params_dim must be equal to params_vector.shape[0]"
    ...
    def compute_update(self, group: Dict[str, Any], A: Tensor, b: Tensor):
        assert A.dim() == 2, "A must be a 2-dimensional matrix"
        assert (b.dim() == 1) or (b.dim() == 2 and b.shape[-1] == 1), "b must be a vector"
        ...
        if block_size <= 0:
            update = (torch.inverse(A) @ b.view(-1, 1)).flatten()
        else:
            try:
                ...
                for i in range((param_dim - 1) // block_size + 1):
                    ...
                    update[current_idx:end_idx] = sub_update.flatten()
    ...
    def step_with_closure(self, closure: Closure) -> float:
        ...
        for group in self.param_groups:
            ...
            assert (residual.dim() == 1) or (residual.dim() == 2 and residual.shape[-1] == 1), \
                "residual must be a vector."
            if residual.dim() == 2 and residual.shape[-1] == 1:
                residual = residual.reshape(-1)
            assert jacobian.shape[0] == residual.shape[0], \
                "Jacobian's row must correspond to residual's elements."

            epsilon = group['epsilon']
            gradient = jacobian.permute(1, 0) @ residual.view(-1, 1)
            ...
            A = hessian + state['miu'] * torch.eye(hessian.shape[0], device=hessian.device)
            update = self.compute_update(group, A, gradient)
            ...
            new_gradient = jacobian.permute(1, 0) @ residual.view(-1, 1)

            varrho = None
            if group['strategy'] == '0':
                varrho = ((last_loss - new_loss) /
                          (0.5 * update @ (state['miu'] * update.view(-1, 1) - gradient))
                          ).item()
            elif group['strategy'] == '1':
                varrho = ((new_loss - last_loss) / (update @ new_gradient)).item()
    ...
    def step_with_jacobians(self, Jacobians: Iterable[Tensor], residual: Tensor) -> float:
        ...
        for group, jacobian in zip(self.param_groups, Jacobians):
            ...
            epsilon = group['epsilon']
            new_gradient = jacobian.permute(1, 0) @ residual.view(-1, 1)
            if (torch.linalg.vector_norm(new_gradient.flatten(), ord=numpy.inf) <= epsilon):
                state['found'] = True
            if 'found' in state:
                if state['found'] == True:
            ...
            if len(state) == 0:
                ...
                if group['strategy'] == '0':
                    varrho = ((last_loss - new_loss) /
                              (0.5 * last_update @ (state['miu'] * last_update.view(-1, 1) - new_gradient))
                              ).item()
                elif group['strategy'] == '1':
                    varrho = ((new_loss - last_loss) / (last_update @ new_gradient)).item()
            ...
            # update temporarily
            A = hessian + state['miu'] * torch.eye(hessian.shape[0], device=hessian.device)
            update = self.compute_update(group, A, new_gradient)
            params_length = torch.linalg.vector_norm(state['params_vector']).item()
            if torch.linalg.vector_norm(update) < epsilon * (params_length + epsilon):
                state['found'] = True
                continue
            self.update(group, update)
            state['gradient'] = new_gradient.flatten()
